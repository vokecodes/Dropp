...
import { RESTAURANT_ORDER_URL, SUB_CHEF_URL } from "../../_redux/urls";
...
const ChefDineIn = () => {
  ...
  const getRestaurantOrders = async (currentPage = page, searchText='') => {
    ...
    SERVER.get(`${SUB_CHEF_URL}/all-restaurant?page=${currentPage}&table=${searchTable}&searchText=${searchText}`)
      .then(({ data }) => {
        if (currentPage === 1) {
  ...
  const dineExportToCSV = async (data: any) => {
    ...
    try {
      ...
      const { data: apiData } = await SERVER.get(
        `${SUB_CHEF_URL}/all-restaurant/download?table=${searchTable}&searchText=${searchText}`
      );
                          ...
                          <div className="text-sm">
                            ...
                            {parseInt(
                              menu?.menu.eventAmount
                                ...
                                : menu?.menu.discount
                                ? menu.menu.price -
                                  (menu.menu.price / 100) * menu.menu.discount
                                : menu.menu.price
                            ).toLocaleString()}
                          </div>
                        </div>
