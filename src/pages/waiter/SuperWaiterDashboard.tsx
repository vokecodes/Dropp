    ...
    try {
      ...
      setHasMore((prev) => {
        ...
        Object.keys(data?.columnCount || {}).forEach((tableId) => {
          ...
          Object.keys(data?.columnCount[tableId] || {}).forEach((status) => {
            const totalCount = data.columnCount[tableId][status]?.totalCount || 0;
            const currentCount =
      ...
      >
                    ...
                    <div className="flex flex-row justify-between items-center">
                      <div className="ml-1 flex flex-row justify-between items-center">
                        ...
                        >
                          ...
                          >
                            {selectedTable && columnCount[selectedTable][cat?.value]}
                          </p>
                        </div>
                      </div>
                    </div>
        ...
        <div className="rounded-2xl h-full w-full lg:p-5 mt-3 flex flex-col lg:flex-row justify-start items-center lg:items-start">
          <div className="w-full lg:w-8/12 h-full flex flex-col gap-x-5">
            <InfinityScroll
              data={selectedTable && filteredColumns[selectedTable][selectedCategory.value] || []}
              getMore={loadMore}
              hasMore={selectedTable && hasMore[selectedTable][selectedCategory.value]}
              ...
              <div className="flex flex-col mt-2">
                {filteredColumns ? (
                  filteredColumns[selectedTable] ? (
                    ...
                    filteredColumns[selectedTable][selectedCategory?.value].length >
                      0 ? (
                      filteredColumns[selectedTable][selectedCategory?.value]?.map(
                        (tableOrder: any, i: number) => {
                          if(selectedCategory?.value !== 'Kitchen' && ['pending', 'completed'].includes(selectedCategory?.value)){
                            return (<OrderItem
                              key={i}
                              order={tableOrder}
                              page={page}
                              ...
                              chef={chef}
                              table={Object.keys(filteredColumns).filter(item => item === selectedTable)[0]}
                            />)
                          }else{
