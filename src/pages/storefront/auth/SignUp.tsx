...
const StorefrontSignUpPage = () => {
  ...
  const registerSchema = Yup.object().shape({
    ...
    storefrontName: Yup.string().required("Storefront name is required."),
    restaurantLocation: Yup.array(Yup.string()).required(
      "Storefront location is required."
    ),
  ...
  const [restaurantLocation, setStorefrontLocation] = useState<any[]>([]);
  ...
  const registerUser = (values: any, formikBag: any) => {
    axios
      .post(`${BASE_API_URL}/auth/storefront/register`, {
        ...values,
      })
      .then(({ data }) => {
        const { success } = data;
