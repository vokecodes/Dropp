...
import {
  customerWithdrawTransactions,
  getCustomerTransactions,
} from "../../_redux/transaction/transactionAction";
...
import { userWithdrawTransaction } from "../../_redux/transaction/transactionCrud";
import {
  getChefRestaurantWalletAccount,
  getChefRestaurantWalletTransactionAccount,
  getChefWalletAccount,
  getChefWalletTransactionAccount,
...
import { chefWalletWithdraw } from "../../_redux/user/userCrud";
...
const ChefWallet = () => {
  ...
  useEffect(() => {
    ...
    dispatch(getChefWalletTransactionAccount());
  ...
  const { values, handleChange, handleSubmit, errors } = useFormik({
    ...
    onSubmit: () => {
      ...
      const walletBalance = restaurantWallet?.balance
        ? wallet?.balance + restaurantWallet?.balance
        : wallet?.balance;

      if (walletBalance < 0 || walletBalance < parseFloat(values.amount)) {
  ...
  const withdrawTransaction = async (recipientCode: any) => {
    ...
    return chefWalletWithdraw(body)
      .then(async ({ data }) => {
      ...
      .catch((err) => {
        ...
        setErrorMessage(
          error?.message ||
            "Something went wrong with the transfer. Please try again and if it persists contact support."
        );
      });
  };
        ...
        <div className="w-full px-6 py-4">
          ...
          <div className="mt-5">
            <BannerCard
              ...
              bgExtraClasses="p-7 lg:p-10 bg-cover bg-no-repeat min-w-full h-"
              ...
              chefWallet={wallet?.balance}
              restaurantWallet={restaurantWallet?.balance}
              ...
              <>
                {walletTransaction?.length > 0 ? (
                  <>
                    ...
                    <div>
                      {walletTransaction?.map((transaction: any, i: number) => (
                        <WalletItem
                          key={i}
