...
import { getAdminDashboardAccount, getAdminMonthlyChart, getChefRestaurantWalletAccount, getChefWalletAccount, getProfileChefAccount, getRestaurantDashboardAccount } from '../../_redux/user/userAction';
...
import { ADMIN_ALL_CUSTOMERS, ADMIN_ALL_ORDERS, ADMIN_ALL_RESTAURANTS } from '../../_redux/urls';
...
const DashboardPage = () => {
  ...
  const { user, dashboardLoading, dashboard, chartData, section, table } = useSelector(
    (state: any) => ({
  ...
  const dashboardItems = [
    {
      title: "Revenue",
      value: (dashboard?.totalNetSales * (2.5 / 100)).toFixed(2),
      toolTipId: "revenue",
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then(({ data }) => {
        ...
        const numOfRestaurants = data.data.filter((item :any, i :any) => item.isRestaurant === true)
        console.log('numOfRestaurants= ', data.data.filter((item :any, i :any) => item.isRestaurant === true))
  ...
  return (
    <AdminDashboardLayout>
      <div className="w-full px-6 py-4 bg-white" style={{}}>
        ...
        <div className="w-full my-10 flex flex-row items-end justify-start flex-wrap gap-3">
          ...
          <div className="w-1/5">
            <div className="w-full h-14 bg-[#F8F8F8] grid grid-cols-2 mt-2 lg:mt-0 p-2 rounded-lg">
              <div className={`w-full h-full flex flex-row items-center justify-center cursor-pointer rounded-lg ${currencyType === 'Naira' ? 'bg_primary text-white' : 'text-black'}`} onClick={() => setCurrencyType('Naira')}>
                <p className='font_medium'>Naira</p>
              </div>
              <div className={`w-full h-full flex flex-row items-center justify-center cursor-pointer rounded-lg ${currencyType === 'Dollars' ? 'bg_primary text-white' : 'text-black'}`} onClick={() => setCurrencyType('Dollars')}>
          ...
          <div className='w-full h-72'>
            <ResponsiveContainer width={"100%"} height={"100%"}>
              <BarChart
                width={730}
                height={250}
                data={chartData[currentChartYear]}
              >
