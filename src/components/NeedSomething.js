...
const NeedSomething = ({ items }) => {
  return (
    <section className="w-4/5 mx-auto pt-5 pb-20">
      ...
      <div className="my-10 lg:flex lg:flex-row lg:flex-wrap lg:justify-between">
        {items?.map((item) => (
          <div className="mr-3" key={item.id}>
            <img src={item.image} alt={item.tile} className="mx-auto" />
            <p className="font_regular text-lg text-black text-center my-3">
              {item.title}
            </p>
          </div>
        ))}
      ...
      <div className="mt-16 text-center">
        <a href="#stores">
          <button className="w-72 font_bold py-5 border border-transparent text-lg rounded-xl shadow-sm text-white bg_primary">
            Schedule a Dropp
          </button>
        </a>
      </div>
