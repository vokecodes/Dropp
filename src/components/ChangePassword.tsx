...
const ChangePassword = ({ showPasswordModal, setShowPasswordModal }: any) => {
  ...
  const updatePassword = (values: any, formikBag: any) => {
    const result = sessionStorage.getItem("auth") || "{}";
    const { token, data } = JSON.parse(result);

    axios
      .patch(
        `${BASE_API_URL}/auth/changePassword/${data?.user?._id}`,
        {
          ...values,
        },
        {
